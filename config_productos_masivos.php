...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_update'])) {
    ...
    if ($percent != 0) {
        ...
        if ($stmt->execute($params)) {
            ...
            $status = "success";
            Logger::event('PRODUCT_MASS_UPDATE', 'catalog', null, ['percent' => $percent, 'cat' => $category, 'brand' => $brand, 'count' => $count]);
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_bulk'])) {
    ...
    if (move_uploaded_file($_FILES["csv_bulk"]["tmp_name"], $target)) {
        ...
        while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
            ...
            $stmt = $db->prepare("UPDATE products SET unit_cost_usd = ?, stock_current = ? WHERE sku = ? AND company_id = ?");
            $stmt->execute([$cost, $stock, $sku, $_SESSION['company_id']]);
            $count += $stmt->rowCount();
        }
        fclose($handle);
...
// Get brands
$db = Vsys\Lib\Database::getInstance();
$brands = $db->query("SELECT DISTINCT brand FROM products WHERE brand IS NOT NULL AND brand != '' AND company_id = " . $_SESSION['company_id'])->fetchAll(PDO::FETCH_COLUMN);
