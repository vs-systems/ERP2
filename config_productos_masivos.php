...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_update'])) {
    ...
    if ($percent != 0) {
        ...
        $sql = "UPDATE products SET unit_cost_usd = unit_cost_usd * (1 + (:percent / 100)) WHERE 1=1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_bulk'])) {
    ...
    if (move_uploaded_file($_FILES["csv_bulk"]["tmp_name"], $target)) {
        ...
        while (($data = fgetcsv($handle, 1000, $sep)) !== FALSE) {
            ...
            if (empty($sku))
                continue;
            $stmt = $db->prepare("UPDATE products SET unit_cost_usd = ?, stock_current = ? WHERE sku = ?");
            $stmt->execute([$cost, $stock, $sku]);
            $count += $stmt->rowCount();
        }
        fclose($handle);
